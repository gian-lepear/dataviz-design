...
| `mostly-one-value` | warn | §1 overview first, §10 data eyeball attack — a table of 12+ rows or a mark group of 20+ where the modal pattern covers 70% (rows) or 75% (marks). Rows are signed by the distinct numeric values they carry, not by their text, so "10 10 10 10" and "10 n/a 10 n/a" count as the same statement. Show what differs and put the rest behind a disclosure |
